...
<context>
用户问题: {question}
涉及集合/表 (来自实际 tool 调用, 不可修改): {collections}
tool_trace 摘要 (按时间序):
{tool_trace_summary}
</context>
...
<task id="2" name="result_summary">用一句自然语言描述做了什么过滤/关联/聚合, 不做具体数值说明.
单表/集合的分组聚合用"按 X 字段分组统计 Y".
单步过滤+排序用"在 X 表/集合上按 Y 条件过滤, 按 Z 排序".
举例: "在 orders 上按 status 字段分组, 统计各状态数量"
举例: "在 orders 上按 user_id 过滤, 按 created_at 降序排列"
</task>
</tasks>

<output>严格 JSON, 无代码围栏, 无额外文本:
{{"question_pattern": "...", "result_summary": "..."}}
</output>
