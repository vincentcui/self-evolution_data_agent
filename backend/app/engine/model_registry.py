...
- 内存缓存 + cfg-keyed 多槽 cache + 双检锁保证并发安全
- Chat 支持运行时热切换; Embedding 仅启动时初始化一次（切换需重嵌入）
- CHAT 支持多 namespace 槽: namespace 级别优先, 未配置时回退全局槽 (NULL)
...
使用示例:
    ...
    # 就绪检查 (支持 namespace 级别)
    ready = registry.is_ready(namespace_id=ns_id)
    # 配置解析 (namespace 优先, 回退全局)
    cfg = registry.resolve_chat_config(namespace_id=ns_id)
    # Chat client (由 llm.py::_get_openai_client 内部调用)
    client = registry.get_chat_client(cfg, namespace_id=ns_id)
...
class ModelRegistry:
    """大模型注册中心: 持有激活配置 + 缓存客户端实例, 支持热切换 + 多 namespace."""
    ...
    def refresh_chat(self, config: dict[str, Any] | None, namespace_id: int | None = None) -> None:
        """更新指定 namespace 的 Chat 配置, 清空对应槽的 client 缓存."""
        with self._lock:
            if config is None:
                self._chat_configs.pop(namespace_id, None)
                self._chat_clients.pop(namespace_id, None)
                self._chat_client_keys.pop(namespace_id, None)
            else:
                self._chat_configs[namespace_id] = config
                self._chat_clients[namespace_id] = None
                self._chat_client_keys[namespace_id] = None
        log.info("[model_registry] Chat 配置已更新: ns=%s model=%s",
                 namespace_id, config.get("model_name") if config else "已清空")
    ...
    def resolve_chat_config(self, namespace_id: int | None) -> dict[str, Any] | None:
        """解析 namespace 级别 Chat 配置, miss 时回退全局.

        Args:
            namespace_id: 请求所属 namespace; None 时直接取全局槽。

        Returns:
            命中的配置 dict, 两级都无配置时返回 None。
        """
        if namespace_id is not None:
            cfg = self._chat_configs.get(namespace_id)
            if cfg is not None:
                return cfg
        return self._chat_configs.get(None)

    # ── 就绪状态查询 ─────────────────────────────────────────

    def is_ready(self, namespace_id: int | None = None) -> dict[str, bool]:
        """返回 Chat / Embedding 是否就绪。namespace_id 非 None 时检查该 namespace (含全局兜底)."""
        chat_ok = self.resolve_chat_config(namespace_id) is not None
        return {
            "chat_ready": chat_ok,
            "embedding_ready": self._embedding_config is not None,
            "ready": chat_ok and self._embedding_config is not None,
        }
    ...
    def get_chat_client(self, cfg: dict[str, Any] | None = None, namespace_id: int | None = None) -> Any:
        ...
        (避免重复读); 不传时内部从 resolve_chat_config 取。
        热切换瞬间进行中的请求可能用旧 config, 下一个请求自动取新 config (最终一致性).
        """
        if cfg is None:
            cfg = self.resolve_chat_config(namespace_id)
        if cfg is None:
            raise RuntimeError(
                "无激活的 Chat 模型配置，请前往「模型管理」页面添加并激活 CHAT 类型配置。"
            )
        return self._get_chat_client(cfg, namespace_id)
    ...
    def _get_chat_client(self, cfg: dict[str, Any], namespace_id: int | None = None) -> Any:
        """懒加载并缓存 Chat 客户端 (线程安全, 多槽 cfg-keyed cache).

        每个 namespace_id 独立维护 client 实例和 cache key,
        不同 namespace 的不同配置不会互相驱逐。
        """
        cfg_key = (cfg.get("protocol", "openai"),
                    cfg.get("api_key"), cfg.get("base_url"))
        existing = self._chat_clients.get(namespace_id)
        existing_key = self._chat_client_keys.get(namespace_id)
        if existing is not None and existing_key == cfg_key:
            return existing
        ...
        with self._lock:
            ...
            if proto == "anthropic":
                ...
                client = build_openai_client(
                    ...
                    timeout=settings.llm_client_timeout_secs, proxy_url=proxy_url)
            self._chat_clients[namespace_id] = client
            self._chat_client_keys[namespace_id] = cfg_key
        return self._chat_clients[namespace_id]
    ...
    async def load_from_db(self) -> None:
        ...
        CHAT 配置按 namespace_id 分组加载到多槽; EMBEDDING 保持全局单槽。
        ...
        try:
            ...
            for row in rows:
                ...
                if row.model_type == "CHAT":
                    self.refresh_chat(cfg, namespace_id=row.namespace_id)
