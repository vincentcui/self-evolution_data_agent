...
TOOL_SPECS: list[dict] = [
    # ── 知识层 ──
    {
        ...
        "description": (
            ...
            "example {question_pattern:语义骨架, collections:[\"database.collection\" 集合全名, 如 shop.orders], "
            ...
            "result_summary?:结果描述}; "
            "route_hint {collection_path:[\"database.collection\" 有序集合路径, 如 [shop.orders, shop.products]], "
            "navigation_note:导航说明(关联字段/关联类型/嵌套位置/避坑)}; "
            "rule {rule_text:规则描述, "
            "rule_kind:business_constraint|filter_default|join_pattern, "
            "applies_to_collections:[\"database.collection\" 集合全名], "
            "priority:优先级(默认0)}. "
            "部分条目另含 references"
