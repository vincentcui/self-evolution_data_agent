...
(非 terminology 特殊路径), payload 原样落库 (json.dumps, 不走 parse_payload 校验),
验证响应 entry.payload 是新 {collection_path, navigation_note} 形态。
"""
...
async def test_create_route_hint_payload_persists_dry_shape(db, admin_client):
    ...
    assert stored["collection_path"] == ["categories", "products", "skus"]
